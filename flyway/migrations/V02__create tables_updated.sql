...
create table Crawler (
	...
	crawlerUserName varchar(50) not null,
	crawlerIsDeleted boolean default false,
	
	constraint crawler_pk primary key (crawlerID),
	constraint crawler_email_uq unique (crawlerEmail),
	constraint crawler_username_uq unique (crawlerUserName)
);
...
create table WebSpins(
	...
	constraint web_spins_spin_fk foreign key (spinID) references Spin(spinID),
...
create table SpinComment(
	...
	constraint spin_comment_crawler_fk foreign key (crawlerID) references Crawler(crawlerID),
...
create table SpinTags(
	...
	constraint spin_tags_spin_fk foreign key (spinID) references Spin(spinID),
...
create table SpinLikes(
	...
	constraint spin_likes_crawler_fk foreign key (crawlerID) references Crawler(crawlerID),
	constraint spin_likes_spin_fk foreign key (spinID) references Spin(spinID),
	constraint spin_likes_crawler_spin_uq unique (crawlerID, spinID)
);
...
create table CommentLikes(
	...
	constraint comment_likes_crawler_fk foreign key (spinCommentID) references SpinComment(spinCommentID),
	constraint comment_likes_spin_fk foreign key (crawlerID) references Crawler(crawlerID),
	constraint comment_likes_spin_comment_crawler_uq unique (spinCommentID, crawlerID)
);